...
from .kb_retrieval import kb_retrieve, parse_query_context
from .retriever import check_zone_exists

# FIX: single module-level constant — was duplicated inside 3 functions,
# risking silent divergence when one copy was updated but not the others.
...
# FIX: status substrings that indicate a value is HIGH (above adequate range)
# but trigger=False.  Old code checked for "ABOVE_ADEQUATE" / "ABOVE ADEQUATE" /
# "ABOVE_RANGE" — strings config.py NEVER produces, so the guard was always empty.
# Actual HIGH labels from config: "HIGH", "HIGH (>25 kg/ha)", "HIGH (>200 kg/ha)",
# "above target range".  Substring match on lowercase covers all of them.
_HIGH_STATUS_SUBSTRINGS = ("high", "above target")
...
def soil_advisory(soil_vals: dict, user_data: dict) -> str:
    ...
    # FIX: match actual status labels from config.py (e.g. "HIGH", "HIGH (>25 kg/ha)",
    # "above target range") — old check used "ABOVE_ADEQUATE" which config never produces.
    above_adequate_params = [
        p for p, i in classified.items()
        if not i["trigger"] and any(s in i.get("status", "").lower() for s in _HIGH_STATUS_SUBSTRINGS)
    ]
    triggered_block = (
    ...
    if above_adequate_params:
        triggered_block += (
            "\nAbove adequate — NO application needed (value is high, NOT deficient):\n"
            + "\n".join(
                f"  {p}: {classified[p]['value']} — {classified[p]['status']} "
                f"(above adequate range, below toxicity threshold)"
                for p in above_adequate_params
            )
        )
    ...
    kb_zone    = _ZONE_ALIASES.get((retrieval_zone or "").lower(), retrieval_zone)
    zone_in_kb = check_zone_exists(kb_zone) if kb_zone else False
    zone_note  = (
...
RESPONSE TEMPLATES:
  ...
  ABOVE_ADEQUATE: "Your [param] of [value] [unit] is above the adequate range but below
                   the toxicity threshold. No further application is recommended."
  HIGH/TOXIC:     "Your [param] of [value] [unit] is above the safe range. Avoid further
                   application — [param] has a narrow safety margin in coffee soils."
...
CRITICAL — DIRECTION RULE: "above adequate" means the value is TOO HIGH, not deficient.
Never describe an ABOVE_ADEQUATE parameter as low, deficient, or requiring supplementation.
"""
...
=========================="""
    ...
    return llm_call(
        ...
        num_predict=700,  # FIX: 1200 exceeds qwen2.5:3b output capacity (~400-600 tokens)
    )
...
def answer_side_question(user_message: str, user_data: dict) -> str:
    ...
    r1_rule = (
        ...
        if measured_str != "None"
            ...
            "state ONLY target ranges that are EXPLICITLY present in KB Context. "
            "Do NOT invent, estimate, or recall ranges from training data."
        )
    )
    ...
    KB_ANCHOR_RANGES = """
KB-ANCHORED TARGET RANGES (use ONLY these — never substitute training-data values):
  pH  : 5.5–6.5
  OC  : 0.75–1.5% (good); above 1.5% = very good in shaded systems
  N   : 280–560 kg/ha = medium/adequate
  P   : 10–25 kg/ha = medium/adequate
  K   : above 280 kg/ha = adequate for bearing coffee
  Zn  : 0.6–2.0 mg/kg = adequate
  B   : 0.5–1.0 mg/kg = adequate
These are the ONLY numeric ranges permitted. If KB Context confirms them, use them.
If KB Context contradicts them, defer to KB Context. Never use any other numbers.
"""
...
6. Answer in concise bullets — one line per parameter. No padding.
7. Write clearly for a farmer — direct, no jargon.
8. Do NOT say "Based on the KB Context".
9. ANTI-HALLUCINATION: Never state a numeric range unless it appears in KB Context
   OR in KB-ANCHORED TARGET RANGES above. Silence is better than an invented number.
10. If KB Context does not mention a parameter's range, omit that parameter entirely
    rather than guessing. Do NOT use phrases like "specific amounts aren't available".
...
def rag_advisory(query: str, user_data: dict) -> str:
    ...
    triggered_params = [p for p, i in classified.items() if i["trigger"]]
    # FIX: same label-mismatch fix as soil_advisory above
    above_adequate_params = [
        p for p, i in classified.items()
        if not i["trigger"] and any(s in i.get("status", "").lower() for s in _HIGH_STATUS_SUBSTRINGS)
    ]
    ...
    # Resolve common aliases before the KB check so "Coorg" doesn't
    # falsely report as missing when the KB stores the same region as "Kodagu".
    kb_zone    = _ZONE_ALIASES.get((retrieval_zone or "").lower(), retrieval_zone)
    zone_in_kb = check_zone_exists(kb_zone) if kb_zone else False
    zone_note  = (
    ...
    TEMPLATES = """
RESPONSE TEMPLATES:
  LOW/DEFICIENT:  "Your [param] of [value] [unit] is below the adequate level."
  ABOVE_ADEQUATE: "Your [param] of [value] [unit] is above the adequate range but below
                   the toxicity threshold. No further application is recommended."
  HIGH/TOXIC:     "Your [param] of [value] [unit] is above the safe range. Avoid further
                   application — [param] has a narrow safety margin in coffee soils."
...
=========================="""
    ...
    return llm_call(system=system_prompt, user=query, num_predict=700)  # FIX: was 1200